...
export async function loadChefContext(opts: { sessionId?: string }) {
  ...
  const baseQ = sb.from("assistant_conversations").select("turn_role,text,created_at").eq("user_id", uid).order("created_at", { ascending: false }).limit(HISTORY_TURNS * 2);
  const { data: turns } = opts.sessionId
    ? await sb.from("assistant_conversations").select("turn_role,text,created_at").eq("user_id", uid).eq("session_id", opts.sessionId).order("created_at", { ascending: false }).limit(HISTORY_TURNS * 2)
    : await baseQ;
  ...
  const { data: memrows } = await sb.from("assistant_memory").select("fact,scope").eq("user_id", uid).is("retired_at", null).order("confirmed_at", { ascending: false }).limit(MEMORY_FACTS);
  const memory: ChefMemoryFact[] = (memrows || []).map((m: any) => ({ fact: m.fact, scope: m.scope }));

  const { data: pairs } = await sb.from("assistant_intents").select("text,confirmed_intent").eq("user_id", uid).not("confirmed_intent", "is", null).order("created_at", { ascending: false }).limit(FEWSHOT_PAIRS);
...
export async function writeTurn(uid: string, fields: { entity_id?: string | null; route?: string | null; session_id?: string | null; turn_role: "user" | "assistant" | "sys"; text: string; intent?: string | null; confidence?: number | null; did_action?: any }) {
  const sb = supabaseServer();
  const { data } = await sb.from("assistant_conversations").insert({ user_id: uid, ...fields }).select("id").maybeSingle();
