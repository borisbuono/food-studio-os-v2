...
export default async function MenuPage() {
  ...
  for (const it of items) {
    const s = (it.section || "other").toLowerCase();
  ...
  return (
    <main className="mx-auto max-w-xl px-6 py-12">
      <Link href="/" className="font-sans text-sm text-ink-soft">← back</Link>
      ...
      {error && <p className="mt-4 font-mono text-sm text-ember">Could not load menu: {error.message}. Check the Supabase env vars on this Vercel project.</p>}
              ...
              return (
                <li key={it.id} className="flex items-baseline justify-between gap-4 py-3">
                  ...
                  <span className="shrink-0 font-mono text-sm text-ink-soft">
                    {it.price ? `€${it.price}` : "–"}
                    {mg !== null && <span className="ml-2 text-olive">{mg}%</span>}
                  </span>
                </li>
              );
            })}
