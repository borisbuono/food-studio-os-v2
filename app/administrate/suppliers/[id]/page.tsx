...
export default async function SupplierHub({ params }: { params: { id: string } }) {
  ...
  // group prices by product name to compute last-move
  ...
  return (
    <main className="mx-auto max-w-xl px-6 py-12">
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">
        <Link href={"/order?supplier=" + p.id} className="rounded-xl bg-ochre px-4 py-3 text-center font-sans text-[14px] font-medium text-[#FCEFE7]">Place an order</Link>
          ...
          <ul className="mt-3 divide-y divide-black/5 rounded-2xl border border-black/10 bg-card">
            {products.slice(0, 30).map((pr: any) => (
              <li key={pr.id} className="flex items-baseline justify-between gap-4 px-4 py-3">
                ...
                <span className="font-mono text-[11px] text-clay">{pr.unit_price ? "€" + Number(pr.unit_price).toFixed(2) + "/" + pr.unit : pr.unit}</span>
              </li>
            ))}
          </ul>
        )}
