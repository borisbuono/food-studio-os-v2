    ...
    <main className="mx-auto max-w-xl px-6 py-12">
      ...
      {w && !saved ? (
        <div className="mt-6 rounded-2xl border border-black/10 bg-card p-5">
          ...
          {!candidate ? <button onClick={attemptSave} disabled={busy || !w.name} className="mt-4 w-full rounded-xl px-6 py-3 font-sans text-[14px] font-medium text-[#FCEFE7] disabled:opacity-60" style={{ background: "var(--accent)" }}>{busy ? "Checking the cellar…" : "Save to cellar"}</button> : null}
        </div>
      ) : null}
      ...
      {candidate ? (
        <div className="mt-4 rounded-2xl border border-tomato/40 bg-tomato/5 p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button onClick={() => updateExisting(candidate.rid, candidate.match)} className="rounded-xl px-4 py-2 font-sans text-[13px] font-medium text-[#FCEFE7]" style={{ background: "var(--accent)" }}>Same wine — update it</button>
