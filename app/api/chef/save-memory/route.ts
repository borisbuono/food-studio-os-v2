...
export async function POST(req: Request) {
  ...
  const { data, error } = await sb.from("assistant_memory").insert({ user_id: u.user.id, fact: String(fact).slice(0, 600), scope: scope || "global", source_conversation_id: source_conversation_id || null, confidence: confidence ?? null }).select("id").maybeSingle();
