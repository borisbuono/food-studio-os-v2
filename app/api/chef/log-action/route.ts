...
export async function POST(req: Request) {
  ...
  const { data, error } = await sb.from("assistant_actions").insert({ user_id: u.user.id, conversation_id: conversation_id || null, action_type, target_table: target_table || null, target_id: target_id || null, payload: payload || null, reversible: !!reversible }).select("id").maybeSingle();
