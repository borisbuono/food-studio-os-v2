  ...
  return (
    <main className="mx-auto max-w-lg px-8 py-16">
      ...
      <div className="mt-8 flex flex-wrap justify-center gap-2">
        {RATINGS.map((r, n) => (
          <button key={n} onClick={() => setRating(n)} className={"rounded-full px-4 py-2 font-sans text-[13px] transition " + (rating === n ? "bg-ember text-[#FCEFE7]" : "border border-black/15 text-ink-soft")}>{r}</button>
        ))}
      </div>
      ...
      <button disabled={rating === null} onClick={() => setSent(true)} className="mt-4 w-full rounded-xl bg-ember px-6 py-4 font-sans text-[15px] font-medium text-[#FCEFE7] disabled:opacity-50">Send feedback</button>
