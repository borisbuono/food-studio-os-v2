...
export default function Onboard() {
  ...
  return (
    <main className="mx-auto max-w-xl lg:max-w-4xl px-6 py-12">
      ...
      <p className="mt-6 font-sans text-xs font-medium text-ink-soft">Onboarding · step {i + 1} of {STEPS.length}</p>
      ...
      <div className="mt-6 flex items-center gap-2">
        {STEPS.map((_, k) => <span key={k} className={"h-1.5 rounded-full transition-all " + (k === i ? "w-8 bg-[color:var(--accent)]" : "w-1.5 bg-black/20")} />)}
      </div>
      ...
      <div className="mt-8 flex gap-3">
        ...
        {!last ? (
          <button onClick={() => setI(i + 1)} className="rounded-xl bg-[color:var(--accent)] px-6 py-3 font-sans text-[14px] font-medium text-[#F7F7F4]">Next</button>
        ) : (
          <Link href="/" className="rounded-xl bg-[color:var(--accent)] px-6 py-3 font-sans text-[14px] font-medium text-[#F7F7F4]">Finish</Link>
        )}
      </div>
