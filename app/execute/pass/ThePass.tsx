...
// The pass board — was app/execute/pass/page.tsx; page.tsx wraps it with
// Metrics as a tab (slim OS slice 4, audit #6).
export default function ThePass() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const restaurant = p?.restaurantId || ENTITY_TO_RESTAURANT[ent] || ENTITY_TO_RESTAURANT[E_BM]!;
      setRid(restaurant);
  ...
  if (!ready) return <main className="mx-auto max-w-xl lg:max-w-4xl px-6 py-6"><p className="font-serif text-2xl text-ink">Opening the pass…</p></main>;
  if (!profile) return (
