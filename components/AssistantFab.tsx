...
export default function AssistantFab() {
  ...
  useEffect(() => {
    ...
    r.onstart = () => { setListening(true); setStatus("Listening — speak now"); };
    r.onresult = (e: any) => {
      let t = "";
      for (let i = 0; i < e.results.length; i++) t += e.results[i][0].transcript;
      setText(t); textRef.current = t; setStatus("Listening — pause to send");
      if (silenceTimer.current) clearTimeout(silenceTimer.current);
      silenceTimer.current = setTimeout(() => { try { r.stop(); } catch {} }, 1400);
    };
    r.onerror = (e: any) => {
      const code = e?.error || "";
      setListening(false); autoRef.current = false;
      if (code === "not-allowed" || code === "service-not-allowed") setStatus("Microphone is blocked — allow mic access for this site in your browser, then tap talk again");
      else if (code === "no-speech") setStatus("Didn’t catch anything — tap talk and speak");
    ...
    r.onend = () => {
      setListening(false);
      if (autoRef.current && textRef.current.trim()) { autoRef.current = false; setStatus(""); sendRef.current(); }
    };
    recRef.current = r;
  }, []);
  ...
  const startListening = () => {
    ...
    setText(""); textRef.current = ""; autoRef.current = true;
    try { r.start(); } catch { /* already running */ }
  };
  const stopListening = () => { const r = recRef.current; if (!r) return; autoRef.current = false; try { r.stop(); } catch {} setListening(false); };
  const toTalk = () => { setMode("talk"); setStatus(supported ? "" : ""); if (supported) setTimeout(startListening, 50); };
  ...
  return (
    <>
      <button onClick={() => setOpen((o) => !o)} aria-label="Assistant" style={{ background: "var(--accent)" }}
      ...
      {open ? (
        <div className="fixed bottom-24 right-5 z-50 flex h-[62vh] max-h-[540px] w-[min(92vw,380px)] flex-col overflow-hidden rounded-2xl border border-black/10 bg-card shadow-2xl shadow-black/25">
          <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 py-5">
            {log.length === 0 && !text ? (
              <p className="font-serif text-[16px] leading-relaxed text-clay">Ask anything — or tap “talk” and just say it. Try: “Order 5 kilos of carrots for tomorrow.”</p>
            ) : null}
            {log.map((m, i) => (
          ...
          <div className="border-t border-black/10 p-3">
            {mode === "type" ? (
              <div className="flex items-center gap-2">
                <input value={text} onChange={(e) => { setText(e.target.value); textRef.current = e.target.value; }}
                ...
                <button onClick={toTalk} className="shrink-0 font-mono text-[11px] uppercase tracking-wide text-clay hover:text-ember">talk</button>
              </div>
            ) : (
              <div className="flex items-center justify-between gap-3">
                <button onClick={() => { stopListening(); setMode("type"); setStatus(""); }} className="font-mono text-[11px] uppercase tracking-wide text-clay hover:text-ember">type</button>
                <div className="flex flex-col items-center">
                  ...
                  <span className="mt-1 max-w-[160px] text-center font-mono text-[9px] uppercase tracking-wide text-clay">{!supported ? "Voice needs Chrome/Safari" : (status || "tap and speak")}</span>
                </div>
