    ...
    <div className="rounded-2xl border border-line bg-card p-6">
      ...
      {inAt ? (
        <>
          ...
          <button onClick={clockOut} className="mt-3 rounded-xl border border-black/15 px-5 py-3 font-sans text-[14px] text-ink-soft transition hover:border-line">Clock out</button>
        </>
      ) : (
        <>
          <p className="mt-2 font-serif text-2xl text-ink">You’re not clocked in</p>
          <button onClick={clockIn} className="mt-3 rounded-xl bg-[color:var(--accent)] px-6 py-3 font-sans text-[14px] font-medium text-[#F7F7F4]">Clock in</button>
        </>
      )}
